...
if (!class_exists('Updraft_Manager_Theme_Updater_1')):
class Updraft_Manager_Theme_Updater_1 {

	public $version = '1.0.0';

	public $relative_theme_path;
	public $slug;
	public $url;
	public $debug;
	...
	public function __construct($mothership, $muid, $relative_theme_path, $options = array()) {
		...
		$this->relative_plugin_file = trailingslashit($relative_theme_path) . 'functions.php';
		$this->slug = dirname($this->relative_plugin_file);
		...
		$this->plugin_file = trailingslashit(get_theme_root()).trailingslashit($relative_theme_path).'style.css';

		if (!file_exists($this->plugin_file)) throw new Exception("Theme file not found: ".$this->plugin_file);
		...
		$this->theme_data = get_file_data( $this->plugin_file, $theme_headers, 'theme' );
		...
		add_filter('site_transient_update_themes', array($this, 'site_transient_update_themes'), 9);
		...
		// add_action("after_plugin_row_$relative_theme_path", array($this, 'after_plugin_row'), 10, 2 );
		// add_action('load-plugins.php', array($this, 'load_plugins_php'));
		add_action('load-themes.php', array($this, 'load_plugins_php'));
	...
	public function get_puc_updates_checker() {
		...
		if (class_exists('Puc_v4_Factory')) {
			$this->plug_updatechecker = Puc_v4_Factory::buildUpdateChecker($this->url, get_theme_root().'/'.$this->relative_plugin_file, $this->slug, $this->interval_hours);
	...
	public function udmupdater_ajax() {
		...
		if ('connect' == $_REQUEST['subaction'] && current_user_can('update_themes')) {
		...
		} elseif ('disconnect' == $_REQUEST['subaction'] && current_user_can('update_themes')) {
	...
	public function admin_menu() {
		...
		if (!empty($this->plug_updatechecker) && !empty($this->plug_updatechecker->optionName) && current_user_can('update_themes')) {
		...
		$plugin_title = htmlspecialchars($this->theme_data['Name']);
		...
		if (is_object($oval) && !empty($oval->update) && is_object($oval->update) && !empty($oval->update->$yourversionkey) && current_user_can('update_themes') && true == apply_filters('udmanager_showcompatnotice', true, $this->slug) && (!defined('UDMANAGER_DISABLECOMPATNOTICE') || true != UDMANAGER_DISABLECOMPATNOTICE)) {
	...
	public function admin_notice_not_connected() {
		echo '<div class="updated udmupdater_not_connected">';
		$plugin_label = htmlspecialchars($this->theme_data['Name']);
		echo apply_filters('udmupdater_updateradminnotice_header', '<h3>'.sprintf(__('Access to plugin updates (%s)', 'udmupdater'), $plugin_label).'</h3>', $this->theme_data);
		$this->print_plugin_connector_box();
		echo '</div>';
		echo "<script>
		jQuery(document).ready(function() {
			if (jQuery('.wp-header-end').length) {
				jQuery('.udmupdater_not_connected').insertAfter('.wp-header-end');
			}
			if (jQuery('form[name=upgrade-themes]').length) {
				jQuery('.udmupdater_not_connected').insertBefore('form[name=upgrade-themes]');
			}
		});
		</script>";
	}
	...
	/**
	 * WordPres action after_plugin_row_($relative_theme_path)
	 *
	 * @param String $file
	 */
						...
						alert('<?php echo esc_js(
							apply_filters('udmupdater_need_credentials_message', 
								sprintf(
									...
									apply_filters('udmupdater_password_description', __('password', 'udmupdater'), $this->slug, $this->theme_data)
								)
							)
						);?>');
						return false;
								...
								if (resp.code == 'INVALID') {
									...
									if (resp.hasOwnProperty('data')) {
										...
										alert('<?php echo esc_js(sprintf(
											__('Your email address and %s were not recognised.', 'udmupdater'), apply_filters('udmupdater_password_description', __('password', 'udmupdater'), $this->slug, $this->theme_data)
										));?>');
										console.log(resp);
									}
								} else if (resp.code == 'OK') {
									alert('<?php echo esc_js(__('You have successfully connected for access to updates to this theme.', 'udmupdater'));?>');
									$('.udmupdater_box_<?php echo esc_js($this->slug);?>').parent().slideUp();
								} else if (resp.code == 'ERR') {
									alert('<?php echo esc_js(__('Your login was accepted, but no available entitlement for this theme was found.', 'udmupdater').' '.__('Has your licence expired, or have you used all your available licences elsewhere?', 'udmupdater'));?>');
									console.log(resp);
								}
							} else {
								alert('<?php echo esc_js(__('The response from the remote site could not be decoded. (More information is recorded in the browser console).', 'udmupdater'));?>');
	...
	protected function print_plugin_connector_box() {
		...
		$plugin_label = htmlspecialchars($this->theme_data['Name']);
		if (!empty($this->theme_data['PluginURI'])) $plugin_label = '<a href="'.esc_attr($this->theme_data['PluginURI']).'">'.$plugin_label.'</a>';
		...
		<div style="margin: 10px;  min-height: 36px;" class="udmupdater_box_<?php echo esc_attr($this->slug);?>">
			...
			<div style="float: left; margin-right: 14px; margin-top: 4px;">
				<em><?php echo apply_filters('udmupdater_entercustomerlogin', sprintf(__('You are connected to receive updates for %s (login: %s)', 'udmupdater'), $plugin_label, htmlspecialchars($email)), $this->theme_data, $this->slug); ?></em>: 
			</div>
			...
			<?php } else { ?>
			<div style="float: left; margin-right: 14px; margin-top: 4px;">
				<em><?php echo apply_filters('udmupdater_entercustomerlogin', sprintf(__('Please enter your customer login to access updates for %s', 'udmupdater'), $plugin_label), $this->theme_data); ?></em>: 
			</div>
			<div class="udmupdater_userpassform udmupdater_userpassform_<?php echo esc_attr($this->slug);?>" style="float:left;">
				<input type="text" style="width:180px;" placeholder="<?php echo esc_attr(__('Email', 'udmupdater')); ?>" name="email" value="">
				<input type="password" style="width:180px;" placeholder="<?php echo esc_attr(ucfirst(apply_filters('udmupdater_password_description', __('password', 'udmupdater'), $this->slug, $this->theme_data))); ?>" name="password" value="">
	...
	public function get_site_info() {
		...
		if (isset($this->theme_data['Version'])) {
			$sinfo['pver'] = $this->theme_data['Version'];
		}
	...
	/**
	 * WordPress filter site_transient_update_themes - used to remove any results from wordpress.org for the same slug.
	 *
